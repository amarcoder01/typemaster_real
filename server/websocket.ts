...
NEVER sound like AI. No "I'd be happy to" or formal language.`
  ...
  // ============================================================================
  // HOST CONTROL HANDLERS (Ready Toggle, Kick Player, Lock Room, Rematch)
  // ============================================================================

  private async handleReadyToggle(ws: WebSocket, message: any) {
    const { raceId, participantId, isReady } = message;
    ...
    const raceRoom = this.races.get(raceId);
    if (!raceRoom) {
      ws.send(JSON.stringify({ type: "error", message: "Race room not found" }));
      return;
    }
    ...
    console.log(`[WS] Ready toggle: ${client.username} (${participantId}) is now ${isReady ? 'ready' : 'not ready'} in race ${raceId}`);
  }
  ...
  private async handleKickPlayer(ws: WebSocket, message: any) {
    ...
    const targetClient = raceRoom.clients.get(targetParticipantId);
    const targetUsername = targetClient?.username || "Player";

    // Add to kicked players set to prevent rejoining
    raceRoom.kickedPlayers.add(targetParticipantId);
    ...
    if (targetClient) {
      ...
      targetClient.ws.send(JSON.stringify({
        type: "kicked",
        message: "You have been removed from the race by the host",
      }));
      
      // Close their connection
      targetClient.ws.close(1000, "Kicked by host");
    }
    ...
    try {
      ...
      console.error(`[WS] Failed to delete kicked participant from DB:`, error);
    }

    // Update cache
    const cachedRace = raceCache.getRace(raceId);
    if (cachedRace) {
      const updatedParticipants = cachedRace.participants.filter(p => p.id !== targetParticipantId);
      raceCache.updateParticipants(raceId, updatedParticipants);
    }

    // Broadcast removal to all remaining participants
    this.broadcastToRace(raceId, {
      type: "participant_kicked",
      participantId: targetParticipantId,
      username: targetUsername,
      message: `${targetUsername} was removed by the host`,
    });

    console.log(`[WS] Host kicked ${targetUsername} (${targetParticipantId}) from race ${raceId}`);
  }

  private async handleLockRoom(ws: WebSocket, message: any) {
    const { raceId, participantId, isLocked, locked } = message;
    // Support both 'isLocked' and 'locked' field names for backwards compatibility
    const lockState = isLocked !== undefined ? isLocked : locked;

    if (!raceId || !participantId || lockState === undefined) {
      ws.send(JSON.stringify({ type: "error", message: "Missing required fields" }));
      return;
    }
    ...
    if (raceRoom.hostParticipantId !== participantId) {
      ws.send(JSON.stringify({ type: "error", message: "Only the host can lock/unlock the room" }));
      return;
    }

    raceRoom.isLocked = lockState;

    this.broadcastToRace(raceId, {
      type: "room_lock_changed",
      isLocked: lockState,
      message: lockState ? "Room locked - no new players can join" : "Room unlocked - new players can join",
    });

    console.log(`[WS] Room ${raceId} ${lockState ? 'locked' : 'unlocked'} by host`);
  }
  ...
  private async handleRematch(ws: WebSocket, message: any) {
    ...
    // Rematch can be requested after the race is finished
    const cachedRace = raceCache.getRace(raceId);
    const race = cachedRace?.race || await storage.getRace(raceId);
    ...
    if (race.status !== "finished") {
      ws.send(JSON.stringify({ type: "error", message: "Race must be finished to create a rematch" }));
      return;
    }

    try {
      // Get current participants to find who wants to rematch
      const currentParticipants = cachedRace?.participants || await storage.getRaceParticipants(raceId);
      const requester = currentParticipants.find(p => p.id === participantId);
      ...
      // Create a new race with similar settings
      const newRace = await storage.createRace({
        ...
        maxPlayers: race.maxPlayers,
        timeLimitSeconds: race.timeLimitSeconds,
        paragraphContent: "", // Will be set when race starts
        paragraphId: null,
      });

      // Notify all participants about the rematch
      const roomClients = raceRoom?.clients;
      if (roomClients) {
        ...
        // Fallback: just send to the requester
        ws.send(JSON.stringify({
          type: "rematch_available",
          newRaceId: newRace.id,
          roomCode: newRace.roomCode,
      ...
      console.log(`[WS] Rematch created by ${requester.username}: race ${newRace.id} (${newRace.roomCode})`);
    } catch (error) {
      console.error("[WS] Rematch creation error:", error);
      ws.send(JSON.stringify({ type: "error", message: "Failed to create rematch" }));
    }
  }
